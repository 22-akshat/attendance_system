...
export const FileUpload: React.FC<FileUploadProps> = ({ onDataLoaded, onOpenPolicy }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-4 md:p-8 bg-gradient-to-b from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 transition-colors">
      <div className="max-w-xl w-full space-y-6">
        ...
        <div
          ...
          className={`relative border-2 border-dashed rounded-3xl p-8 sm:p-12 text-center cursor-pointer transition-all duration-200 ${
            isDragging
              ...
              : 'border-slate-300 dark:border-slate-700/80 bg-white/80 dark:bg-slate-900/60 hover:border-indigo-400 dark:hover:border-slate-600 hover:bg-white dark:hover:bg-slate-900/90 shadow-xl'
        ...
        >
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="space-y-1.5">
              {isLoading ? (
                <div>
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{loadingMessage || 'Processing workbook...'}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Computing shift windows &amp; rollups</p>
                </div>
              ) : (
                <>
                  <p className="text-base font-semibold text-slate-900 dark:text-slate-200">
                    <span className="text-indigo-600 dark:text-indigo-400 hover:underline">Click to browse</span> or drag and drop
                  </p>
                  <p className="text-xs text-slate-600 dark:text-slate-400 max-w-md mx-auto">
                    Upload your monthly attendance Excel file (<code className="font-mono text-slate-800 dark:text-slate-300 font-semibold">.xlsx</code>) to compute punctuality, overtime, and deductions.
                  </p>
                </>
              )}
            </div>
        ...
        {errorMessage && (
          <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800/80 rounded-2xl p-4 flex items-start gap-3 text-xs text-red-800 dark:text-red-200 animate-in fade-in">
            <AlertCircle className="w-5 h-5 text-red-600 dark:text-red-400 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <p className="font-bold text-red-900 dark:text-red-300">Failed to Parse Workbook</p>
              <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{errorMessage}</p>
