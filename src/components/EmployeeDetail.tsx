...
export const EmployeeDetail: React.FC<EmployeeDetailProps> = ({
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 max-w-7xl mx-auto w-full relative">
      {/* Back Button & Breadcrumbs */}
      <div className="flex items-center justify-between">
        <button
          onClick={onBack}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-slate-300 hover:text-white transition"
        >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-semibold text-slate-200 hover:text-white transition"
          >
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 md:p-6 relative overflow-hidden shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start sm:items-center gap-4">
            {/* Avatar Initials */}
            <div className="w-14 h-14 rounded-2xl bg-indigo-600 border border-indigo-400/40 flex items-center justify-center font-black text-xl text-white shadow-lg shadow-indigo-600/20 shrink-0">
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {employee.note && (
                  <span className="text-xs px-2.5 py-0.5 rounded-full bg-amber-950/80 text-amber-300 border border-amber-800/80 font-medium">
                    {employee.note}
                  </span>
                )}
                {s.editedCount > 0 && (
                  <span className="text-xs px-2.5 py-0.5 rounded-full bg-amber-950/80 text-amber-300 border border-amber-800/80 font-medium flex items-center gap-1">
          ...
          <div className="flex sm:flex-col items-center sm:items-end justify-between border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-800 shrink-0">
            <span className="text-xs text-slate-400 font-medium">Net Day Adjustment</span>
            <div className="flex items-center gap-2 mt-1">
              <span
                className={`text-2xl sm:text-3xl font-black font-mono px-3 py-0.5 rounded-xl border ${
                  s.netDaysAdjustment < 0
                    ? 'bg-red-950/80 text-red-400 border-red-800/70'
                    : s.netDaysAdjustment > 0
                    ? 'bg-blue-950/80 text-blue-400 border-blue-800/70'
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3.5 space-y-1">
          <span className="text-[11px] font-medium text-slate-400">Present Days</span>
          <div className="text-xl font-extrabold text-emerald-400">{s.present}</div>
          <div className="text-[10px] text-slate-500">Out of {s.workingDays} working</div>
        </div>

        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3.5 space-y-1">
          <span className="text-[11px] font-medium text-slate-400">Absent Days</span>
          <div className="text-xl font-extrabold text-red-400">{s.absent}</div>
          <div className="text-[10px] text-slate-500">Unapproved absence</div>
        </div>

        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3.5 space-y-1">
          <span className="text-[11px] font-medium text-slate-400">Leaves Taken</span>
          <div className="text-xl font-extrabold text-amber-400">{s.leave}</div>
          <div className="text-[10px] text-slate-500">Approved leaves</div>
        </div>

        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3.5 space-y-1">
          <span className="text-[11px] font-medium text-slate-400">Weekly Offs</span>
          <div className="text-xl font-extrabold text-slate-300">{s.weeklyOff}</div>
          <div className="text-[10px] text-slate-500">Sundays / W-Off</div>
        </div>

        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3.5 space-y-1">
          <span className="text-[11px] font-medium text-slate-400">Attendance Rate</span>
          <div className="text-xl font-extrabold text-indigo-400">{s.attendanceRatePct}%</div>
          <div className="text-[10px] text-slate-500">Present / (P+A+L)</div>
        </div>

        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3.5 space-y-1">
          <span className="text-[11px] font-medium text-slate-400">Total Deductions</span>
          <div className="text-xl font-extrabold text-red-400">-{s.totalDeductionDays.toFixed(1)}d</div>
          <div className="text-[10px] text-slate-500">Late + Early + Half</div>
        </div>
      </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-200">Late Arrivals</span>
            </div>
            <span className="text-xs font-bold font-mono text-slate-100 bg-slate-800 px-2 py-0.5 rounded">
              {s.lateCount} days
            </span>
          </div>
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex items-center justify-between text-slate-300">
              ...
              <span className="text-slate-400 font-mono">3 days max</span>
            </div>
            <div className="flex items-center justify-between text-slate-300">
              <span>Penalty Rate:</span>
              <span className="text-slate-400">1 day per late &gt; 3</span>
            </div>
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between font-bold">
              <span className="text-slate-200">Late Deductions:</span>
              <span className="text-red-400 font-mono">-{s.lateDeductionDays} days</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-200">Early Departures</span>
            </div>
            <span className="text-xs font-bold font-mono text-slate-100 bg-slate-800 px-2 py-0.5 rounded">
              {s.earlyCount} days
            </span>
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <div className="flex items-center justify-between text-slate-300">
              ...
              <span className="text-slate-400 font-mono">E-30 to E-16</span>
            </div>
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between font-bold">
              <span className="text-slate-200">Early Deductions:</span>
              <span className="text-orange-400 font-mono">-{s.earlyDeductionDays} days</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-200">Half Days</span>
            </div>
            <span className="text-xs font-bold font-mono text-slate-100 bg-slate-800 px-2 py-0.5 rounded">
              {s.halfDayCount} days
            </span>
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex items-center justify-between text-slate-300">
              ...
              <span className="text-purple-300 font-mono">{s.halfDaySlot1Count}</span>
            </div>
            <div className="flex items-center justify-between text-slate-300">
              <span>Slot 2 (very late &amp; out&le;3h30m):</span>
              <span className="text-purple-300 font-mono">{s.halfDaySlot2Count}</span>
            </div>
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between font-bold">
              <span className="text-slate-200">Half-Day Deductions:</span>
              <span className="text-purple-400 font-mono">-{s.halfDayDeductionDays.toFixed(1)} days</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-200">Overtime Accrual</span>
            </div>
            <span className="text-xs font-bold font-mono text-blue-400 bg-blue-950 px-2 py-0.5 rounded border border-blue-800/60">
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <div className="flex items-center justify-between text-slate-300">
              ...
              <span className="text-slate-400 font-mono">{s.remainderOtMinutes} mins</span>
            </div>
      ...
      {anomalousDays.length > 0 && (
        <div className="bg-violet-950/30 border border-violet-700/60 rounded-2xl p-5 space-y-3 shadow-md animate-in fade-in">
          ...
          <p className="text-xs text-violet-200/80 leading-relaxed">
            The following records have malformed or irregular timestamps (such as non-positive duration, under 1h on present day, or missing punches). Click "Manual Edit / Resolve" to correct them:
          </p>
          <div className="space-y-2 pt-1">
            {anomalousDays.map(d => (
              <div
                key={d.day}
                className="bg-slate-950/80 border border-violet-800/50 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs"
              >
                <div className="flex items-center gap-3">
                  <span className="px-2.5 py-1 rounded-md bg-violet-900/60 text-violet-200 font-mono font-bold">
                    Day {d.day} ({d.dow})
                  </span>
                  <span className="text-slate-200 font-medium">
                    In: <code className="text-slate-300 font-mono">{d.in || 'Blank'}</code> &bull; Out: <code className="text-slate-300 font-mono">{d.out || 'Blank'}</code>
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-violet-300 font-medium text-[11px] flex items-center gap-1.5">
                    <AlertTriangle className="w-3.5 h-3.5 text-violet-400 shrink-0" />
                    <span>{d.anomalyReason || 'Irregular entry — check manually'}</span>
                  </div>
                  <button
                    onClick={() => startEditing(d)}
                    className="px-2.5 py-1 rounded-lg bg-indigo-600/80 hover:bg-indigo-600 text-white font-semibold text-[11px] flex items-center gap-1 transition"
                  >
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        <button
          ...
          className="w-full p-4 flex items-center justify-between text-left hover:bg-slate-800/50 transition select-none"
        >
        ...
        {showPolicyPanel && (
          <div className="p-5 border-t border-slate-800 bg-slate-950/60 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3 space-y-1">
              ...
              <div className="text-[10px] text-slate-500">Up to 15 min after shift start</div>
            </div>
            ...
            <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3 space-y-1">
              ...
              <div className="text-[10px] text-slate-500">16 - 30 min after start</div>
            </div>
            ...
            <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3 space-y-1">
              ...
              <div className="text-[10px] text-slate-500">Bidirectional Case A &amp; Case B</div>
            </div>
            ...
            <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-3 space-y-1">
              ...
              <div className="text-[10px] text-slate-500">1 OT Day = {(w.otDayThresholdMins/60).toFixed(1)} hrs logged</div>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden shadow-sm space-y-4 p-5">
        ...
        {/* VIEW 1: Calendar Grid (7 columns) */}
        {activeView === 'calendar' && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {Array.from({ length: firstDayOfWeekIndex }).map((_, idx) => (
                <div
                  key={`empty-${idx}`}
                  className="min-h-[95px] rounded-xl border border-slate-800/30 bg-slate-950/20 opacity-30"
                />
              ))}
              ...
              {employee.days.map(day => {
                ...
                if (day.status === 'P') {
                  bgClass = 'bg-emerald-950/20 border-emerald-800/40 hover:border-emerald-500';
                  statusText = 'Present';
                } else if (day.status === 'A') {
                  bgClass = 'bg-red-950/20 border-red-800/40 hover:border-red-500';
                  statusText = 'Absent';
                } else if (day.status === 'L') {
                  bgClass = 'bg-amber-950/20 border-amber-800/40 hover:border-amber-500';
                  statusText = 'Leave';
                } else if (day.status === 'WO') {
                  bgClass = 'bg-slate-800/30 border-slate-800 hover:border-slate-600';
                  statusText = 'Weekly Off';
                } else if (day.status === 'NA') {
                  bgClass = 'bg-slate-950/30 border-dashed border-slate-800 text-slate-600';
                ...
                return (
                  <div
                    ...
                    } ${day.edited ? 'ring-1 ring-amber-500/40' : ''}`}
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 p-0.5 rounded bg-slate-800 hover:bg-indigo-600 text-slate-300 hover:text-white transition"
                    ...
                    {day.status === 'P' ? (
                      <div className="text-[11px] font-mono text-slate-300 space-y-0.5 my-1">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-slate-500">In:</span>
                          <span className={`${day.arrivalTag === 'Late' || day.arrivalTag === 'VeryLate' ? 'text-amber-400 font-bold' : 'text-slate-200'}`}>
                            {day.in || '-'}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-slate-500">Out:</span>
                          <span className={`${day.departureTag === 'Early' ? 'text-orange-400 font-bold' : 'text-slate-200'}`}>
                            {day.out || '-'}
                          </span>
                        </div>
                      </div>
                    ) : (
                      <div className="text-[10px] text-slate-500 text-center my-auto">
                        {statusText}
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-1 pt-1">
                      {day.edited && (
                        <span
                          title={day.editReason ? `Reason: ${day.editReason}` : 'Manually edited'}
                          className="text-[9px] font-bold px-1 rounded bg-amber-950 text-amber-300 border border-amber-800 flex items-center gap-0.5"
                        >
                          <Pencil className="w-2 h-2" /> Edited
                        </span>
                      )}
                      {day.arrivalTag === 'Late' && (
                        <span className="text-[9px] font-bold px-1 rounded bg-amber-950 text-amber-300 border border-amber-800">
                          Late
                        </span>
                      )}
                      {day.isHalfDay && (
                      ...
                      {day.departureTag === 'Early' && !day.isHalfDay && (
                        <span className="text-[9px] font-bold px-1 rounded bg-orange-950 text-orange-300 border border-orange-800">
                          Early
                        </span>
                      )}
                      {day.otMinutes > 0 && (
                        <span className="text-[9px] font-bold px-1 rounded bg-blue-950 text-blue-300 border border-blue-800">
                          +{day.otMinutes}m OT
                        </span>
                      )}
                      {day.dataAnomaly && (
                        <span className="text-[9px] font-bold px-1 rounded bg-violet-950 text-violet-300 border border-violet-800 flex items-center gap-0.5">
                    ...
                    {isHovered && !editingDay && (
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-slate-900 border border-slate-700 rounded-xl p-3 shadow-2xl z-30 text-xs pointer-events-none animate-in fade-in zoom-in-95">
                        ...
                        <div className="space-y-1.5 pt-2 text-[11px]">
                          ...
                          {day.status === 'P' && (
                            <>
                              ...
                              <div className="flex justify-between">
                                <span className="text-slate-400">Arrival Tag:</span>
                                <span className={`font-semibold ${day.arrivalTag === 'Late' || day.arrivalTag === 'VeryLate' ? 'text-amber-400' : 'text-emerald-400'}`}>
                              ...
                              <div className="flex justify-between">
                                <span className="text-slate-400">Departure Tag:</span>
                                <span className={`font-semibold ${day.departureTag === 'Early' ? 'text-orange-400' : 'text-emerald-400'}`}>
                          ...
                          {day.dataAnomaly && (
                            <div className="pt-1.5 border-t border-slate-800 text-violet-300 font-semibold">
                              &bull; {day.anomalyReason}
                            </div>
                          )}

                          {day.edited && (
                            <div className="pt-1.5 border-t border-slate-800 text-amber-300 font-medium">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
                {employee.days.map(day => (
                  <tr key={day.day} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-bold text-slate-200 flex items-center gap-1.5">
                      <span>{day.day}</span>
                      {day.edited && (
                        <span title={day.editReason || 'Manually edited'} className="text-amber-400">
                          <Pencil className="w-2.5 h-2.5" />
                        </span>
                      )}
                    </td>
                    <td className="py-2.5 px-3 text-slate-400">{day.date}</td>
                    <td className="py-2.5 px-3 text-slate-400">{day.dow}</td>
                    ...
                    <td className="py-2.5 px-3 text-center text-slate-200">{day.in || '-'}</td>
                    <td className="py-2.5 px-3 text-center">
                      {day.arrivalTag === 'Late' || day.arrivalTag === 'VeryLate' ? (
                        <span className="text-amber-400 font-bold">{day.arrivalTag}</span>
                      ) : day.arrivalTag ? (
                        <span className="text-emerald-400 font-semibold">{day.arrivalTag}</span>
                      ) : (
                        <span className="text-slate-600">-</span>
                      )}
                    </td>
                    <td className="py-2.5 px-3 text-center text-slate-200">{day.out || '-'}</td>
                    <td className="py-2.5 px-3 text-center">
                      {day.departureTag === 'Early' || day.departureTag === 'VeryEarly' ? (
                        <span className="text-orange-400 font-bold">{day.departureTag}</span>
                      ) : day.departureTag ? (
                        <span className="text-emerald-400 font-semibold">{day.departureTag}</span>
                      ) : (
                        <span className="text-slate-600">-</span>
                      )}
                    </td>
                    <td className="py-2.5 px-3 text-center">
                      {day.isHalfDay ? (
                        <span className="text-purple-400 font-bold">{day.halfDaySlot}</span>
                      ) : (
                        <span className="text-slate-600">-</span>
                      )}
                    </td>
                    <td className="py-2.5 px-3 text-center font-bold text-blue-400">
                    ...
                    <td className="py-2.5 px-3 font-sans text-xs">
                      {day.dataAnomaly ? (
                        <span className="text-violet-300 font-medium flex items-center gap-1">
                          <AlertTriangle className="w-3 h-3 text-violet-400 shrink-0" />
                    ...
                    <td className="py-2.5 px-3 text-center">
                      <button
                        onClick={() => startEditing(day)}
                        className="px-2 py-1 rounded bg-slate-800 hover:bg-indigo-600 text-slate-300 hover:text-white text-xs font-sans transition flex items-center gap-1 mx-auto"
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4 text-xs">
              ...
              {editStatus === 'P' && (
                <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-300">In-Time (HH:MM)</label>
                    <input
                      type="text"
                      placeholder="e.g. 10:05"
                      value={editIn}
                      onChange={e => setEditIn(e.target.value)}
                      className="w-full px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-slate-100 font-mono placeholder:text-slate-600 focus:outline-none focus:border-indigo-500"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-300">Out-Time (HH:MM)</label>
                    <input
                      type="text"
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="font-semibold text-slate-300">Manual Classification Override</label>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="font-semibold text-slate-300">
                    Reason for change {editOverride !== 'auto' && <span className="text-amber-400 font-bold">* (Required)</span>}
                  </label>
                </div>
                <textarea
                  rows={2}
                  ...
                  className={`w-full px-3 py-2 rounded-xl bg-slate-950 border text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500 resize-none ${
                    editOverride !== 'auto' && !editReason.trim() ? 'border-amber-700/80 bg-amber-950/10' : 'border-slate-700'
                  }`}
                />
              </div>
